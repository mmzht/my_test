...
def get_file(file_path):
    ...
    for roots, dirs, file_names in os.walk(file_path):
        for name in file_names:
            ...
            if file_suf.lower() == '.jpg':
                ...
                phash = imagehash.whash(img)  # phash
    ...
    # with open(os.path.join(file_path, 'Img_whash_Data.dat'), 'wb') as f:
...
def file_rename_move(sim_list, file_path):
    ...
    for sim_files in sim_list:
        ...
        num += 1
    group_df = pd.DataFrame(group_list, columns=('分组', '文件名', '新位置'))
